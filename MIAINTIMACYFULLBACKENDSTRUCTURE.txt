...
const CompanionProfileSchema = new mongoose.Schema({
  ...
  voice: {
    ...
    fillers: [{ type: String }], // "um", "like", "you know"
...
class SentimentAnalyzer {
  ...
  analyze(text) {
    ...
    // Short responses = neutral/closed off
    if (text.length < 10) {
      scores.neutral += 3;
    }
...
class MemoryEngine {
  ...
  async getRelevantMemories(userId, currentMessage, sentiment, limit = 5) {
    ...
    const categoryPriority = ['intimacy', 'emotional_pattern', 'fact', 'preference', 'conversation_thread', 'inside_joke'];
    ...
    // Filter by relevance to current sentiment
    const relevant = recentMemories.filter(mem => {
      ...
      if (mem.accessCount > 3) return true; // Frequently referenced
...
// ═══════════════════════════════════════════════════════════════
// CONTEXT ENGINE - Mood & Strategy Detection
// ═══════════════════════════════════════════════════════════════
...
class ContextEngine {
  ...
  async detectState(userId, userMessage, conversationHistory, memories) {
    ...
    if (messageCount > 0 && conversationHistory[conversationHistory.length - 1]?.role === 'assistant') {
      strategy = 'deepen_connection'; // Don't ask another question
    }

    // Determine intimacy level (1-5)
    let currentIntimacyLevel = 1;
    const lastIntimateMessage = conversationHistory.slice().reverse().find(m => m.intimacyLevel > 1);
    if (lastIntimateMessage) currentIntimateLevel = lastIntimateMessage.intimacyLevel;
    
    // Escalate based on user intent
    if (escalation.level > currentIntimacyLevel) {
      currentIntimacyLevel = Math.min(escalation.level, currentIntimacyLevel + 1); // Never jump more than 1
    }
...
// ═══════════════════════════════════════════════════════════════
// RESPONSE VARIATOR - Anti-Boring System
// ═══════════════════════════════════════════════════════════════

class ResponseVariator {
  constructor() {
    this.usedVariants = new Map(); // userId -> Set of used variants
...
// ═══════════════════════════════════════════════════════════════
// INTIMACY LADDER - Escalation Management
// ═══════════════════════════════════════════════════════════════
...
class IntimacyLadder {
  ...
  canEscalate(currentLevel, userIntent, userProfile) {
    ...
    // Require emotional foundation for high intimacy
...
class OpenAIService {
  ...
  async generateResponse(messages, temperature = 0.85, maxTokens = 300) {
    try {
      const response = await this.openai.chat.completions.create({
        ...
        frequency_penalty: 0.4, // Discourage same phrases
        top_p: 0.9
      });
  ...
  async generateVoiceText(text, voiceStyle = 'warm') {
    // This prepares text for ElevenLabs
    // Add breathing markers, pauses, emotional cues
    const enhanced = text
      .replace(/\.{3}/g, '... [pause] ')
      .replace(/,/g, ', [breath] ')
      .replace(/\?/g, '? [soft] ')
...
// ═══════════════════════════════════════════════════════════════
// MAIN CHAT ENGINE - Orchestrates Everything
// ═══════════════════════════════════════════════════════════════
...
class MiaChatEngine {
  ...
  async processMessage(userId, userMessage, companionName = 'Mia') {
    ...
    // 9. Post-process: Anti-boring checks
    ...
    // 11. Extract and store new memories
...
- End with a statement, an image, or silence. Never a question.`;
  ...
  postProcessResponse(text, userMessage, state, memories, userName) {
    ...
    // 1. Remove question endings if not appropriate
    if (state.phase !== 'opening' && state.phase !== 'confused') {
      processed = processed.replace(/\\?$/g, '.');
      processed = processed.replace(/\\?\\s*$/g, '.');
    }

    // 2. Inject personalization
    if (userName) {
  ...
  async storeConversation(userId, userMessage, responseText, sentiment, intimacyLevel) {
    ...
    // Check if session expired (30 min gap)
...
print("✅ mia-engine.js created (complete backend engine)")
print(f"   Size: {len(mia_engine)} characters")
